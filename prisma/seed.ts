  ...
  const readers = await Promise.all([
    ...
    prisma.tarotReader.create({
      data: {
        ...
        price: 95,
        rating: 4.91,
        bio: 'Бывший юнгианский терапевт, ставший консультантом. Джеймс мастерски раскрывает скрытые мотивации и повторяющиеся паттерны, блокирующие рост.',
        imageUrl: null,
      },
    }),
  ])
...
Расклад, казавшийся неуместным три месяца назад, может стать разительно ясным, когда вы к нему вернётесь. Ведите записи. Те, что вас запутали, со временем часто становятся самыми поучительными.`,
    ...
    prisma.insightArticle.create({
      data: {
        ...
        preview: 'Что отличает консультанта Foundation от Мастера? Не только годы — но и глубина, которую каждый привносит в разные вопросы.',
        content: `Выбор консультанта — это не только бюджет. Каждый уровень предлагает качественно иное, и правильный выбор зависит от того, через что вы проходите.
...
Начинайте там, где живёт ваш вопрос. Консультант Foundation с чётким вопросом превзойдёт Мастера с расплывчатым — всегда.`,
        category: 'guide',
        readTime: 5,
      },
    }),
    prisma.insightArticle.create({
      data: {
        ...
        content: `«Когда это произойдёт?» — один из самых частых вопросов консультантам и один из самых сложных для честного ответа.
...
Говорите, что замечаете. Если слова консультанта не попадают — скажите об этом. Это полезная информация. Если что-то бьёт точно — тоже скажите. Живой расклад точнее, когда он действительно отзывчив.
